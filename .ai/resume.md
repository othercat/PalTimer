...
2026-07-27 本轮改动：PAL98/PAL98DX9 的“水灵珠”节点不再依赖正常获取时可能被 70ms 轮询漏掉的瞬时脚本状态。正常路线在场景 `267`、小李逍遥交换点 `(1040,1640)` 的 `r=4` 范围内锁定当前水灵珠数量 `N`，后续轮询只有看到数量大于 `N` 才触发；没有前置水灵珠只是 `N=0` 的特例。十年前之前随机物品/特殊剧情使背包成为任意 `N` 不会触发，正常交换后的 `N→N+1` 才触发。离开范围会废弃未完成基线。回梦无痕同样按“触发前任意 `N`、卡牌给予后通常为 `N+1`”建模，但由于增量也可能被70ms轮询漏掉，仍在跳到大理后使用最终 `SSS.MKF`/`M.MSG` 动态解析的大理祭坛返回对话兜底。聚焦状态机与有效资源检查通过，待 PAL98/PAL98DX9 双路线实机确认。不欢乐模式没有该节点，未修改。
...
- PAL98/PAL98DX9 的“水灵珠”节点不再由 `0x109` 物品单独触发；正常路线要求先进入场景 `267` 的小李逍遥交换范围并在后续轮询看到物品数量增长，大理祭坛路线则要求动态解析的返回对话结束且背包存在水灵珠
- 新增 `Pal98WaterSpiritPearlSplit.cs` 与 `.ai/water_spirit_pearl_split_regression_check.ps1`；当前有效资源确认交换中心 `(1040,1640)`、范围 X±64/Y±32，大理分支解析为 `FFFF 2AB1`/脚本 `773F`；70ms Observe 路径无文件 I/O
...
- **PAL98/PAL98DX9 水灵珠双路线节点**：坐标基线状态机、有效资源检查与构建均通过。仍需分别实机验证：十年前之前由随机物品/特殊剧情使背包成为任意 `N` 时不跳节点，进入交换范围锁定 `N` 后，正常交换的 `N→N+1` 才跳；无前置水灵珠时是 `N=0` 的特例；回梦无痕从任意 `N` 给予一颗并跳过十年前后，在大理祭坛“小李子”对话结束且物品存在时跳；F10 reset 后旧门闩不残留。PAL98UNHAPPY 未改。
...
1. 实机验证 PAL98/PAL98DX9 水灵珠节点三场景：十年前之前随机物品/特殊剧情使背包成为任意 `N` 时不跳，进入交换范围锁定 `N` 后由正常交换的 `N→N+1` 跳；无前置水灵珠时按 `N=0` 验证；回梦无痕从任意 `N` 再给予一颗并跳到大理，确认不能在卡牌增量时提前跳，只在“小李子”对话结束后跳，并补测离开/重进交换范围和 F10 reset 清门闩。
...
| `Pal98Timer/Pal98WaterSpiritPearlSplit.cs` | 水灵珠节点坐标基线增长检测、大理对话动态解析与双路线门闩 |
...
### 2026-07-27 会话（水灵珠节点坐标基线 + 大理剧情门闩）

- PAL98/PAL98DX9 正常路线在场景 `267`、交换中心 `(1040,1640)`、X±64/Y±32 范围内记录当前水灵珠数量；只有后续仍在范围内读到数量增长才完成节点，避免 70ms 轮询漏掉约 10ms 的脚本指令
- 十年前之前由随机物品/特殊剧情形成的任意水灵珠数量 `N` 位于交换范围外，不触发节点；之后进入范围只记录基线 `N`，交换脚本再次给予后读到大于 `N`（通常 `N+1`）才触发。离开范围后获得的物品不会沿用旧基线误触发，重新进入会重建基线
- 回梦无痕触发前也允许已有任意 `N`；卡牌执行 `001F 0109` 后通常成为 `N+1` 并跳过十年前。计时器不依赖捕捉这次快速增量，而是继续匹配大理祭坛后的“糟．．希望灵儿不会有事才好”；每次连接新 PAL.exe 从最终 `M.MSG` 文本和 `SSS.MKF` 唯一 `FFFF` 引用解析该分支
- 70ms OnTick 复用既有场景/坐标/背包快照，并沿 `0x428000 -> p1 + 0x500 -> p2` 只读 8 字节脚本状态；无资源文件热循环 I/O。大理资源解析失败只关闭回梦无痕分支，正常坐标路线仍可工作
- 行为 harness 覆盖当前有效资源坐标、动态大理 ID、同 PID 缓存、换 PID 重解析、十年前前任意 `N`、正常 `N→N+1`、离开/重进范围、回梦无痕 `N→N+1` 后的大理兜底、无关对话、缺少物品、reset、重复/损坏资源
...
```text
REAL: normal=area 267/center (1040,1640)/radius (64,32), Dali=0x2AB1/script=0x773F
PASS: 有效资源交换坐标、动态大理 ID、同 PID 一次解析缓存、换 PID 重解析、损坏/重复拒绝、十年前前任意 N 不触发、范围内正常 N→N+1、离开/重进范围、回梦无痕 N→N+1 后大理兜底、缺少物品、reset、无关对话保护。
