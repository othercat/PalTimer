...
- 新增 `Pal98WaterSpiritPearlSplit.cs` 与 `.ai/water_spirit_pearl_split_regression_check.ps1`；有效资源确认正常中心 `(1040,1640)`、大理回程中心 `(1168,760)`，两者范围均为 X±96/Y±48；运行时无资源解析或脚本状态读取
...
1. 实机验证 PAL98/PAL98DX9 水灵珠节点三场景：十年前之前随机物品/特殊剧情使背包成为任意 `N` 时不跳，进入场景267扩大交换范围锁定 `N` 后由正常交换的 `N→N+1` 跳；回梦无痕从任意 `N` 再给予一颗并跳到大理，确认只在场景204回程扩大范围内跳。两个分支补测范围边缘、范围外、离开/重进和 F10 reset。
...
### 2026-07-27 会话（水灵珠节点双扩大位置门闩）

- PAL98/PAL98DX9 正常路线在场景 `267`、交换中心 `(1040,1640)`、X±96/Y±48 扩大范围内记录当前水灵珠数量；只有后续仍在范围内读到数量增长才完成节点，避免70ms轮询漏掉短暂状态
- 十年前之前由随机物品/特殊剧情形成的任意水灵珠数量 `N` 位于交换范围外，不触发节点；之后进入范围只记录基线 `N`，交换脚本再次给予后读到大于 `N`（通常 `N+1`）才触发。离开范围后获得的物品不会沿用旧基线误触发，重新进入会重建基线
- 回梦无痕触发前也允许已有任意 `N`；卡牌执行 `001F 0109` 后通常成为 `N+1` 并跳过十年前。计时器在场景 `204`、回程中心 `(1168,760)`、X±96/Y±48 扩大范围内结合物品存在触发，不再检测大理对话
- 70ms OnTick 只复用既有 `Area/X/Y` 和背包快照；已删除运行时 `SSS.MKF/M.MSG` 解析、进程附加资源入口及 `p1+0x500` 当前脚本状态读取
- 行为 harness 覆盖有效资源双坐标、十年前前任意 `N`、正常 `N→N+1`、离开/重进、两个扩大范围的边界/范围外、回梦无痕位置+物品、缺少物品和reset
...
```text
REAL: normal=area 267/center (1040,1640)/radius (96,48), Dali=area 204/center (1168,760)/radius (96,48)
PASS: 任意 N、正常扩大范围 N→N+1、离开/重进、大理扩大位置+物品、缺少物品、范围外、reset，以及运行时零脚本/资源依赖。
PASS: 70ms Observe 只使用既有场景/坐标/背包快照，PAL98UNHAPPY 未接入。
