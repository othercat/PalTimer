...
2026-07-27 本轮改动：PAL98/PAL98DX9 的“水灵珠”节点不再依赖正常获取时可能被 70ms 轮询漏掉的瞬时脚本状态。正常路线在场景 `267`、小李逍遥交换点 `(1040,1640)` 的 `r=4` 范围内锁定当前水灵珠数量，后续轮询只有看到数量增长才触发：普通路线为范围内 `0→1`；十年前之前随机物品/特殊剧情已造成 `0→1` 时，该次增长不触发，进入范围锁定基线 `1`，正常交换后的 `1→2` 才触发。离开范围会废弃未完成基线。回梦无痕仍使用最终 `SSS.MKF`/`M.MSG` 动态解析的大理祭坛返回对话。聚焦状态机与有效资源检查通过，待 PAL98/PAL98DX9 双路线实机确认。不欢乐模式没有该节点，未修改。
...
- **PAL98/PAL98DX9 水灵珠双路线节点**：坐标基线状态机、有效资源检查与构建均通过。仍需分别实机验证：十年前之前由随机物品/特殊剧情发生 `0→1` 时不跳节点，进入交换范围锁定基线 `1` 后，正常交换的 `1→2` 才跳；无前置水灵珠时范围内 `0→1` 正常跳；回梦无痕跳过十年前后在大理祭坛“小李子”对话结束且物品存在后跳；F10 reset 后旧门闩不残留。PAL98UNHAPPY 未改。
...
1. 实机验证 PAL98/PAL98DX9 水灵珠节点三场景：十年前之前随机物品/特殊剧情的 `0→1` 不跳，进入交换范围锁定 `1` 后由正常交换的 `1→2` 跳；无前置水灵珠时范围内 `0→1` 跳；回梦无痕路线在大理祭坛“小李子”对话结束后跳，并补测离开/重进交换范围和 F10 reset 清门闩。
...
- 十年前之前由随机物品/特殊剧情发生的 `0→1` 位于交换范围外，不触发节点；之后进入范围只记录基线 `1`，交换脚本再次给予后读到 `2` 才触发。离开范围后获得的物品不会沿用旧基线误触发，重新进入会重建基线
- 回梦无痕跳过十年前路线仍匹配大理祭坛后的“糟．．希望灵儿不会有事才好”；每次连接新 PAL.exe 从最终 `M.MSG` 文本和 `SSS.MKF` 唯一 `FFFF` 引用解析该分支
- 70ms OnTick 复用既有场景/坐标/背包快照，并沿 `0x428000 -> p1 + 0x500 -> p2` 只读 8 字节脚本状态；无资源文件热循环 I/O。大理资源解析失败只关闭回梦无痕分支，正常坐标路线仍可工作
- 行为 harness 覆盖当前有效资源坐标、动态大理 ID、同 PID 缓存、换 PID 重解析、开局已有物品、`0→1`、`1→2`、离开/重进范围、回梦无痕、无关对话、缺少物品、reset、重复/损坏资源
...
PASS: 有效资源交换坐标、动态大理 ID、同 PID 一次解析缓存、换 PID 重解析、损坏/重复拒绝、十年前前置 0→1 不触发、范围内正常 0→1、前置后正常 1→2、离开/重进范围、跳过路线、缺少物品、reset、无关对话保护。
