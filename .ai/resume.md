...
2026-07-27 本轮改动：PAL98/PAL98DX9 的“水灵珠”节点改为双位置门闩，彻底移除脚本状态依赖。实机在场景267确认旧 `X/Y=(912,1512)` 是视口坐标、实际地图坐标 `rX/rY=(1072,1624)`，最近的小李逍遥对象位于 `(1088,1616)`、TriggerMode=2；正常路线现按实际坐标和队伍朝向复现该对象的手动触发搜索范围，并在范围内锁定水灵珠数量 `N`，后续轮询只有看到数量大于 `N` 才触发。回梦无痕在场景 `204`、实际回程落点 `(1168,760)` 的 X±96/Y±48 扩大范围内，结合背包存在水灵珠触发。70ms循环只复用既有场景、实际坐标、朝向和背包快照，不再解析 `SSS.MKF`/`M.MSG`，也不再读取当前脚本状态。聚焦状态机与有效资源检查通过，待 PAL98/PAL98DX9 双路线实机确认。不欢乐模式没有该节点，未修改。
...
- PAL98/PAL98DX9 的“水灵珠”节点不再由 `0x109` 物品单独触发；正常路线要求进入场景267的扩大交换范围并在后续轮询看到数量增长，回梦无痕要求进入场景204的扩大回程范围且背包存在水灵珠
- 新增 `Pal98WaterSpiritPearlSplit.cs` 与 `.ai/water_spirit_pearl_split_regression_check.ps1`；实机确认旧 `X/Y` 是视口坐标，正常分支现读取 `rX/rY` 与朝向，匹配场景267小李逍遥对象 `(1088,1616)` 的 TriggerMode=2 手动触发范围；大理回程使用实际坐标 `(1168,760)` 的 X±96/Y±48；运行时无资源解析或脚本状态读取
...
- **PAL98/PAL98DX9 水灵珠双路线节点**：实际地图坐标/朝向状态机、有效资源检查与构建均通过。仍需分别实机验证：十年前之前由随机物品/特殊剧情使背包成为任意 `N` 时不跳，靠近场景267的小李逍遥并处于其 TriggerMode=2 手动触发范围后锁定 `N`，正常交换的 `N→N+1` 才跳；回梦无痕从任意 `N` 给予一颗并跳到大理后，在场景204扩大回程范围内跳；两个分支都要补测范围边缘、范围外和F10 reset。PAL98UNHAPPY 未改。
...
1. 实机验证 PAL98/PAL98DX9 水灵珠节点三场景：十年前之前随机物品/特殊剧情使背包成为任意 `N` 时不跳；靠近场景267的小李逍遥并面向可交互方向，确认进入其 TriggerMode=2 手动触发范围后锁定 `N`，由正常交换的 `N→N+1` 跳；回梦无痕从任意 `N` 再给予一颗并跳到大理，确认只在场景204实际回程扩大范围内跳。两个分支补测触发范围边缘、范围外、离开/重进和 F10 reset。
...
- 回梦无痕触发前也允许已有任意 `N`；卡牌执行 `001F 0109` 后通常成为 `N+1` 并跳过十年前。计时器在场景 `204`、实际回程中心 `(1168,760)`、X±96/Y±48 扩大范围内结合物品存在触发，不再检测大理对话
- 70ms OnTick 只复用既有 `Area/rX/rY/PartyDirection` 和背包快照；已删除运行时 `SSS.MKF/M.MSG` 解析、进程附加资源入口及 `p1+0x500` 当前脚本状态读取
- 行为 harness 覆盖有效资源、小李逍遥四方向手动触发点、TriggerMode=2 最远允许点/首个禁止点、十年前前任意 `N`、正常 `N→N+1`、离开/重进、大理实际坐标范围、回梦无痕位置+物品、缺少物品和reset
...
```text
REAL: normal=area 267/child (1088,1616)/trigger mode 2, Dali=area 204/center (1168,760)/radius (96,48)
PASS: 任意 N、小李逍遥四方向手动触发点、TriggerMode=2 边界、离开/重进、大理实际坐标+物品、reset，以及运行时零脚本/资源依赖。
