...
PALDLL v1.63 配套计时器提供两个独立的原版 DX9 配置：**仙剑98柔情DX9-1.2秒**保留原来的 DX9 配置身份和成绩；**仙剑98柔情DX9-0.8秒**使用独立的最佳成绩、时间线、导出和接力文件。右上角设置菜单选择对应配置，游戏的黑屏模式仍需在游戏配置工具中设置并重启生效。
...
成绩 JSON 记录 `TimerCore`、选择的 `TimingModeMs` 和实际的 `PaletteFadeModeMs`。跨分类接力在写入 RPG 或附属文件前拒绝；旧版未标注分类的接力只兼容原 DX9 1.2 秒配置。云端请求也使用各自核心身份，服务端是否接受新身份需要服务端配套验证；本次未登录验证云服务。

模式来自当前游戏进程实际加载的配置，不根据可能已被修改的磁盘 INI 猜测；游戏重启后重新读取。上述两个原版 DX9 配置需要 PALDLL v1.63 或更新版本发布的模式接口；模式未知或不匹配时显示原因并暂停计时，不能保存成已确认的该类成绩。选择正确模式后沿用原有计时算法、节点、F9 和反作弊语义。

魂牵、梦幻22显血和不欢乐保留现有核心；它们与原版 DX9 的 0.8 秒版本/比赛名仍带 `-0.8s` 后缀。UTF-8 成绩和配置支持有 BOM 或无 BOM，便于简繁系统交换；旧 ANSI 文件按本机代码页读取，不能自动推断任意跨区 GBK/Big5 文件。

目前支持自动计时的游戏（点击主界面右上方的设置按钮可以选择切换）：

1. 仙剑98柔情版比赛专用版（补丁版本2.0.10.3、3.0.2014.628）
2. 仙剑98柔情DX9-1.2秒、仙剑98柔情DX9-0.8秒（独立原版时间线）
