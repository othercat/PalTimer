...
1. 特定游戏可以显示特定信息
1. 显示节点之间区间时差
1. 支持跳图路线的非顺序节点推进（仙剑98系列）
1. 节点完成快慢提示音效配置（仙剑98系列）
...
# 更新说明
## v3.36.3 (2026-05-10 by othercat)
修复节点音效试听无反馈的问题（添加MCI错误检查和提示）
修复MP3播放兼容性问题（MCI mpegvideo驱动不可用时自动回退到Windows Media Player COM播放）
修复音效配置窗口快捷键提示文字被截断的问题
回退主计时居中改动，恢复为固定偏移+左对齐布局（居中方案在部分场景下毫秒位置异常）

## v3.36.2 (2026-05-10 by othercat)
节点完成时可配置快慢提示音效，支持 wav/mp3 外部音频文件，功能菜单--节点音效配置可设置每种情况的音效并试听
新增音效开关快捷键，可在运行中一键切换节点音效启用/关闭，并支持配置打开和关闭时的提示音
土灵符实时战斗检测，未偷到时默认不显示
修复游戏关闭后反复弹出"窗口句柄无效"错误
修复F11改键功能在部分旧版改键器上失效的问题（兼容"改建器"和"改键器"两种窗口标题）
~~主计时居中显示~~（v3.36.3 已回退，恢复原布局）
支持跳图路线的非顺序节点推进，被跳过节点显示灰色，再次经过时恢复为正常颜色
