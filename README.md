...
定版预期 SHA-256 清单编译进计时器，不读取旁边的 JSON 作为认证依据。文件扫描由单一后台任务分块执行，完整一轮后间隔至少 5 分钟再复核；节点处理不执行文件读取、哈希或等待核验。需先冻结游戏核心及图形链，再重编计时器并记录最终 EXE 哈希，最后进行真实云激活。云 ID 激活不等同于游戏无篡改证明。实施边界和验证见 [v1.69 核验说明](docs/V169_RUNTIME_INTEGRITY.md)。

游戏正常关闭后清除“核验读取失败”等即时连接提示，已确认的数值、随机数、文件和代码异常仍随跑次保留；游戏仍运行时的诊断失效继续记录和检查关键代码。
...
重启权限提示修正：拒绝访问不再直接视为管理员运行；先通过只读进程查询和令牌确认是否存在提权差异，退出中的旧进程不提示，连接恢复清除过时访问错误。已确认的权限不匹配仍保留原提示。此修正不改变版本号但会改变 EXE 哈希；已经激活旧文件的用户可继续用旧文件，方便重新激活时再覆盖修复版。
...
|---|---|---|---|
| DX9-1.2秒 | `bestPAL98DX9.txt` | `SRPG.bin` | 保留原 DX9 成绩与旧选择记录 |
| DX9-0.8秒 | `bestPAL98DX9_800.txt` | `SRPG.PAL98DX9_800.bin` | 初始最佳时间为零，可独立编辑；不复制 1.2 秒成绩 |
| DX9-0.8秒&快走速 | `bestPAL98DX9_800_SPEED.txt` | `SRPG.PAL98DX9_800_SPEED.bin` | 初始最佳时间为零，独立编辑、保存、重开 |

最佳线文件位于 `Pal98Timer.exe` 所在目录。节点的 `name` 是路线识别键，保持不变；`des` 是显示昵称，可在“编辑最佳成绩”中修改。编辑保存后立即更新当前显示及比较时间，重开仍生效。带日期后缀的 `best*.txt` 是归档备份，直接修改归档不会改变正在使用的最佳线。要采用归档，先退出计时器并备份上表中相同核心的活动文件，再将归档副本命名为该活动文件名后启动；不要将三类文件交叉替换。

保存成绩成功后，会刷新当前核心的最佳时间线，再询问是否重置；随机物品、随机物品＋吴强、随机技能和其它允许计时的玩法同样可用。原最佳线保留为带日期和唯一标识的备份；写入或替换失败保留当前跑次和原文件，不提示成功、不自动重置。各核心仍使用上表中各自的文件，同一核心的新保存会替换其当前最佳线。历史 `Records` 导出原样保留，不自动迁移。最佳线仅用作节点时间和昵称参考，不会把其中的内容身份导入新跑次。

云功能在游戏身份有效后自动初始化，先开计时器也可等待游戏启动。暂时失败自动退避重试，连续四次未成功后停止自动重试；点击“云”菜单可查看原因并“重新验证”。重置、切核心期间会等待旧请求实际结束，旧回调不会写入新跑次。服务端的版本/哈希校验保持原样，验证被拒绝时显示返回原因。配套云组件缺失或哈希不匹配会拒绝初始化；请保留完整发行目录。当前开发侧只有离线调度测试，真实服务端接受情况单独验收。
...
2. 仙剑98柔情DX9-1.2秒、仙剑98柔情DX9-0.8秒、仙剑98柔情DX9-0.8秒&快走速（三条独立原版时间线）
...
1. 游戏接力存档和云存档
1. PalTimer 启动后会在进程生命周期内发布 `Local\PAL98.PalTimer.TournamentLock.v1` 能力标记，供锁定比赛的 PALDLL 在游戏启动时确认计时器支持比赛配置；因此比赛流程先启动 PalTimer、再启动游戏。PAL98DX9、魂牵、Dream220 显血和不欢乐内核会校验游戏目录的 `PAL98.TournamentLock.v1`；有效锁在版本位置只显示来自配置工具“比赛/补丁名称”的精确“xx比赛专用”，不追加普通版本号。它接受最多 8 字符的锁定者名称及成对签名的逐行覆盖选择与配置码标识字段；先前已签名的 v1 清单继续兼容，半升级、签名、字段或快照损坏时不采用该身份。该能力标记和清单读取不修改 PAL98_IPC、内存地址、轮询、节点或计时语义。
...
当前版本直接提供两个可分别选择的标准顶层窗口，不需要安装 OBS 插件：

1. 主计时器窗口：在 OBS 的“窗口采集”中选择标题为“自动计时器”的窗口。功能菜单的“OBS窗口采集样式”可启用逐像素透明模式；“框架透明度”支持 0-100，0 时背景、按钮胶囊、节点底色和图标等框架层完全透明，只保留文字、计时数字及文字描边。该设置同样作用于简版。建议启用前先在同一子菜单配置“样式开关快捷键”，这样即使功能按钮完全透明，也能一键恢复普通界面。
2. PAL98DX9 独立遮罩：在 PAL98DX9 内核菜单中启用“OBS 独立遮罩窗口”，OBS 再单独选择标题为“仙剑98自动计时器 - OBS独立遮罩”的窗口。遮罩不要求游戏窗口位于前台，也不随游戏最小化而隐藏；在计时器菜单中进入调整状态后可拖动和等比缩放，OBS 内仍可继续裁剪和缩放此来源。

两个功能都默认关闭，旧目录缺少 `obs_window_style` 或 `dx9_overlay` 时保持原界面。主窗口样式、框架透明度和样式开关快捷键写入 `obs_window_style`；独立遮罩的位置、比例、字体、颜色与快捷键继续写入 `dx9_overlay_layout`。两类快捷键都默认未设置，必须包含 Ctrl、Shift 或 Alt，并拒绝 F1-F12、Ctrl+Enter、节点音效开关及彼此冲突；按住基准键不会连续反复切换。OBS 侧如需保留透明通道，应在窗口采集来源中启用相应的透明捕获选项；不同 OBS 版本和捕获方法仍需实际直播机复核。
...
## v3.37.0 (2026-08-23, updated 2026-09-03 by othercat)
...
仅 PAL98DX9 内核提供的实验性“游戏内信息叠加”已升级为“OBS 独立遮罩窗口”，可在功能菜单手动启用；缺少 `dx9_overlay` 配置时默认关闭
独立遮罩每 100ms 读取一次计时器已有的只读快照，使用固定窗口标题并作为可由 OBS 单独枚举的顶层窗口存在；不再要求 PAL98DX9 游戏窗口位于前台，也不会因游戏最小化而隐藏。首次启用优先落在当前游戏窗口或主屏幕右下角。功能菜单点开子菜单可启用/关闭、配置遮罩开关快捷键、拖动和等比缩放、调整字体/字号/颜色或恢复默认。进入位置调整时显示低饱和编辑遮罩，左键拖动整体、拖动右下角手柄等比缩放，右键或再次点击菜单完成，完成后恢复无激活和鼠标穿透。绝对屏幕位置、比例、字体、颜色与遮罩快捷键保存到独立的 `dx9_overlay_layout`；旧 version=1 布局仍可读取，首次调整后升级为 version=2
...
关闭功能时只在 PAL98DX9 内核初始化时读取一次小型布局文件以取得快捷键，不创建遮罩窗口和刷新定时器，不新增线程、轮询、`OpenProcess`、`ReadProcessMemory` 或节点读取；当前计时、暂停、反作弊、读档、云存读档和节点推进链保持不变
主窗口新增默认关闭的“OBS 纯文字/透明框架模式”，完整界面和简版共用 `obs_window_style`。该模式用 Win32 逐像素透明窗口保持文字不透明，框架透明度 0-100 只作用于背景图、按钮/图标、节点底色等非文字层；同一子菜单可配置全局样式开关快捷键，即使框架完全透明也能恢复普通界面。快捷键复用现有全局键盘钩子和按键锁存，不新增线程或轮询；失败时自动关闭该模式并恢复普通界面。它不改变窗口整体 `Opacity`，不启动网络，也不接入旧 OBS 插件
本版不承诺旧 OBS CLRHost 兼容性；现代 OBS 的实际透明通道、窗口枚举和多显示器拖动仍需在直播机做人工验收。PALDLL_DX9 的 `PAL98_IPC_v1` 保留为 TAS/诊断候选数据源，本版遮罩不读取或修改其 ABI
