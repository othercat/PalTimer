...
本文记录 PalTimer 3.37.2 的玩家可见名称、内部身份、profile 读取合同和发布门。它只描述公开工具可用的最小兼容信息，不包含 PALDLL_DX9、配置工具或私有资源包源码。
...
五者以 `仙剑98柔情DX9` 的 PAL.exe 读取实现为基类。三个原版核心共用节点条件，分别保存最佳时间；两类 0.8 秒不复制旧成绩和参考时间，接力文件为 `SRPG.PAL98DX9_800.bin`、`SRPG.PAL98DX9_800_SPEED.bin`，旧 1.2 秒仍为 `SRPG.bin`。使用本次 v1.63 的 `PAL98.TimingMode.v1` 核验实际黑屏和走速；未知/不符时暂停，开局后类别改变须重置，六种跨类导入在写入存档前拒绝。旧格式保持原归属，参考时间线可独立编辑，不构成已验证成绩。导出字段和英文榜单名见 README。魂牵和梦幻保留各自路线、节点与存档长度。旧 `梦幻22 / DREAM22` 仍连接 `sdlpal`，不与新核心合并。
