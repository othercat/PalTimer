...
四者都以 `仙剑98柔情DX9` 的 PAL.exe 读取实现为基类。两个原版核心共用节点触发条件，分别保存最佳时间；0.8 秒不复制旧成绩和默认参考时间，接力文件为 `SRPG.PAL98DX9_800.bin`，旧 1.2 秒仍为 `SRPG.bin`。它们使用 PALDLL v1.63 模式接口，未知或模式不符时暂停，跨分类接力在写入存档前拒绝。魂牵和梦幻保留各自路线、节点与存档长度。旧 `梦幻22 / DREAM22` 仍是连接 `sdlpal` 的历史内核，不与新梦幻显血核心合并。
